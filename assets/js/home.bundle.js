    ...
    if (windowWidth <= 768) {
      $searchBtn.on('click', function (e) {
        if ($searchField.val() === '') {
          ...
          $header.toggleClass('search-animation-start');
        }
      });
    }
  });
              ...
              if (i < data.length) {
                for (i; i < i2; i++) {
                  var $template = '<div class="media border-bottom flex-column width-full py-2">' +
                    ...
                    '<p class="g-font-primary-light g-color-grey g-font-size-13 mb-0">a créé ' + data[i].discussionsQty + ' discussions</p>' + data[i].description +
                    '</div>' +
                    '</div>',
                ...
                if (i3 < data.length) {
                  ...
                  if (currentCat === '*' || currentCat === '.' + data[i3].category.slug) {
                    for (i3; i3 < i4; i3++) {
                      ...
                      newAddedItem.find('.rs-badge').tooltip();
                    }
                  }
                }
              });
  ...
  $(document).on('ready', function () {
    $filterItem.on('click', function () {
      ...
      $grid.on('arrangeComplete', function (event, filteredItems) {
        $('.dashboard__items_inner').mCustomScrollbar('update');
      });
